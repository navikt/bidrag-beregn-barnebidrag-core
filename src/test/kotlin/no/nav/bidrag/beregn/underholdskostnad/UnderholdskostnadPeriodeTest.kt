...
internal class UnderholdskostnadPeriodeTest {
    ...
    @Test
    @DisplayName(
        "Test at barnetrygd ikke trekkes fra i barnets fødselsmåned, og at barnetrygd trekkes fra i påfølgende periode som normalt",
    )
        ...
        val sjablonPeriodeListe =
            listOf(
                SjablonPeriode(
                    ...
                    sjablon =
                    Sjablon(
                        navn = SjablonTallNavn.ORDINÆR_BARNETRYGD_BELØP.navn,
                        nokkelListe = emptyList(),
                        innholdListe =
                        listOf(
                            SjablonInnhold(navn = SjablonInnholdNavn.SJABLON_VERDI.navn, verdi = BigDecimal.valueOf(1054)),
                        ),
                    ),
                ),
                SjablonPeriode(
                    ...
                    sjablon =
                    Sjablon(
                        navn = SjablonTallNavn.FORHØYET_BARNETRYGD_BELØP.navn,
                        nokkelListe = emptyList(),
                        innholdListe =
                        listOf(
                            SjablonInnhold(navn = SjablonInnholdNavn.SJABLON_VERDI.navn, verdi = BigDecimal.valueOf(1354)),
                        ),
                    ),
                ),
            )
    ...
    @Test
    @DisplayName(
        "Test forhøyet barnetrygd. Barnet fyller 6 år etter at forhøyet barnetrygd er innført, men fødselsdato settes til 1/7. Gir ikke forhøyet barnetrygd",
    )
    fun testForhoyetBarneTrygd1() {
        ...
        assertAll(
            Executable { assertThat(resultat).isNotNull() },
            Executable {
                assertThat(resultat.resultatPeriodeListe.size).isEqualTo(2)
            ...
            Executable {
                assertThat(resultat.resultatPeriodeListe[0].resultat.belop.compareTo(BigDecimal.valueOf(2000 - 1054))).isZero()
    ...
    fun testForhoyetBarneTrygd2() {
        ...
        assertAll(
            Executable { assertThat(resultat).isNotNull() },
            Executable {
                assertThat(resultat.resultatPeriodeListe.size).isEqualTo(3)
            }, // Første periode er før innføring av forhøyet barnetrygd -> ordinær barnetrygd brukes
            Executable { assertThat(resultat.resultatPeriodeListe[0].periode.datoFom).isEqualTo(LocalDate.parse("2021-03-01")) },
            Executable { assertThat(resultat.resultatPeriodeListe[0].periode.datoTil).isEqualTo(LocalDate.parse("2021-07-01")) },
            Executable {
            ...
            Executable {
                assertThat(resultat.resultatPeriodeListe[1].resultat.belop.compareTo(BigDecimal.valueOf(2000 - 1354))).isZero()
            }, // Forhøyet barnetrygd er innført, barnet regnes som 6 år fordi fødselsdato settes til 01.07. -> ordinær barnetrygd brukes
    ...
    fun testGrunnlagMedAvvik() {
        ...
        assertAll(
            ...
            Executable { assertThat(avvikListe).hasSize(11) },
            Executable {
                assertThat(
                    avvikListe[0].avvikTekst,
                ).isEqualTo("Første dato i barnetilsynMedStonadPeriodeListe (2018-01-01) er etter beregnDatoFom (2015-01-01)")
            },
            ...
            Executable {
                ...
                ).isEqualTo("Første dato i nettoBarnetilsynPeriodeListe (2016-01-01) er etter beregnDatoFom (2015-01-01)")
            },
            ...
            Executable {
                ...
                ).isEqualTo("Første dato i sjablonPeriodeListe (2018-01-01) er etter beregnDatoFom (2015-01-01)")
            },
            Executable { assertThat(avvikListe[4].avvikType).isEqualTo(Avvikstype.PERIODE_MANGLER_DATA) },
        )
    }

    private fun lagGrunnlag(beregnDatoFra: String, beregnDatoTil: String) = BeregnUnderholdskostnadGrunnlag(
    ...
    private fun lagSjablonGrunnlag() = listOf(
        ...
        SjablonPeriode(
            ...
            Sjablon(
                ...
                nokkelListe = listOf(SjablonNokkel(navn = SjablonNøkkelNavn.ALDER_TOM.navn, verdi = "18")),
                innholdListe =
                listOf(
                    SjablonInnhold(navn = SjablonInnholdNavn.FORBRUK_TOTAL_BELØP.navn, verdi = BigDecimal.valueOf(6985)),
                ),
            ),
        ),
        SjablonPeriode(
            ...
            Sjablon(
                navn = SjablonNavn.FORBRUKSUTGIFTER.navn,
                nokkelListe = listOf(SjablonNokkel(navn = SjablonNøkkelNavn.ALDER_TOM.navn, verdi = "5")),
                innholdListe =
                listOf(
                    SjablonInnhold(navn = SjablonInnholdNavn.FORBRUK_TOTAL_BELØP.navn, verdi = BigDecimal.valueOf(3661)),
                ),
            ),
        ),
        SjablonPeriode(
            ...
            Sjablon(
                navn = SjablonNavn.FORBRUKSUTGIFTER.navn,
                nokkelListe = listOf(SjablonNokkel(navn = SjablonNøkkelNavn.ALDER_TOM.navn, verdi = "99")),
                innholdListe =
                listOf(
                    SjablonInnhold(navn = SjablonInnholdNavn.FORBRUK_TOTAL_BELØP.navn, verdi = BigDecimal.valueOf(6985)),
                ),
        ...
        SjablonPeriode(
            ...
            Sjablon(
                navn = SjablonNavn.FORBRUKSUTGIFTER.navn,
                nokkelListe = listOf(SjablonNokkel(navn = SjablonNøkkelNavn.ALDER_TOM.navn, verdi = "10")),
                innholdListe =
                listOf(
                    SjablonInnhold(navn = SjablonInnholdNavn.FORBRUK_TOTAL_BELØP.navn, verdi = BigDecimal.valueOf(5113)),
                ),
            ),
        ),
        SjablonPeriode(
            ...
            Sjablon(
                navn = SjablonNavn.FORBRUKSUTGIFTER.navn,
                nokkelListe = listOf(SjablonNokkel(navn = SjablonNøkkelNavn.ALDER_TOM.navn, verdi = "14")),
                innholdListe =
                listOf(
                    SjablonInnhold(navn = SjablonInnholdNavn.FORBRUK_TOTAL_BELØP.navn, verdi = BigDecimal.valueOf(6099)),
                ),
            ),
        ),
    )
