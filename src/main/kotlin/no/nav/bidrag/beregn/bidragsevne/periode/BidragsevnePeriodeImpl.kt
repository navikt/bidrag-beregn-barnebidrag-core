...
class BidragsevnePeriodeImpl(private val bidragsevneberegning: BidragsevneBeregning) : FellesPeriode(), BidragsevnePeriode {
    ...
    private fun justerDatoerGrunnlagslister(
        ...
        beregnBidragsevneListeGrunnlag.justertInntektPeriodeListe =
            periodeGrunnlag.inntektPeriodeListe
                .map { InntektPeriode(it) }

        beregnBidragsevneListeGrunnlag.justertSkatteklassePeriodeListe =
            periodeGrunnlag.skatteklassePeriodeListe
                .map { SkatteklassePeriode(it) }

        beregnBidragsevneListeGrunnlag.justertBostatusPeriodeListe =
            periodeGrunnlag.bostatusPeriodeListe
                .map { BostatusPeriode(it) }

        beregnBidragsevneListeGrunnlag.justertBarnIHusstandPeriodeListe =
            periodeGrunnlag.barnIHusstandPeriodeListe
                .map { BarnIHusstandPeriode(it) }

        beregnBidragsevneListeGrunnlag.justertSaerfradragPeriodeListe =
            periodeGrunnlag.saerfradragPeriodeListe
                .map { SaerfradragPeriode(it) }

        beregnBidragsevneListeGrunnlag.justertSjablonPeriodeListe =
    ...
    private fun lagBruddperioder(periodeGrunnlag: BeregnBidragsevneGrunnlag, beregnBidragsevneListeGrunnlag: BeregnBidragsevneListeGrunnlag) {
        ...
        beregnBidragsevneListeGrunnlag.bruddPeriodeListe =
            Periodiserer()
